...
const EventList = () => {
  const { bookInfoState, bookingListState, loadingState } =
  ...
  useEffect(() => {
    ...
    fetch("https://redlight-server.herokuapp.com/bookings")
      .then((res) => res.json())
      .then((data) => {
  ...
  const handleStatusChange = (e, { _id, name, email, serviceName }) => {
    ...
    fetch("https://redlight-server.herokuapp.com/updateStatus", {
      method: "PUT",
          ...
          <table class="table table-striped">
            ...
            <tbody>
              ...
              {bookingList.map((list) => {
                const { _id, name, email, serviceName, status, paymentMethod } =
                  list;
                return (
                  <tr key={_id}>
