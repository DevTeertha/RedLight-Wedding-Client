...
const BookingList = () => {
    ...
    useEffect(() => {
        fetch(`https://radiant-earth-05632.herokuapp.com/userBookingList?email=${userInfo.email}`)
            .then(res => res.json())
        ...
        <div className="container-fluid">
            ...
            <div className="border my-3 p-3 bg-white">
                <div className="w-100 p-3 row">
                    {
                        userBookingList.length==0 && <h2 className="text-secondary">No Bookings Found</h2>
                    }
                    {
                        userBookingList.map(list => <BookingListCard list={list} key={list._id}></BookingListCard>)
                    }
                </div>
            </div>
        </div>
