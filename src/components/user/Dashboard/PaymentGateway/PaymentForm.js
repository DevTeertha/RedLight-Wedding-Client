...
const PaymentForm = () => {
  const { serviceState, userInfoState, bookInfoState, loadingState } =
  ...
  const handleSubmit = async (event) => {
    ...
    const { description, price, img } = service.find(
      (sd) => sd.title === bookInfo.serviceName
    );
    ...
    if (!error) {
      if (bookInfo.serviceName === "" || bookInfo.paymentMethod === "") {
        ...
        fetch("https://redlight-server.herokuapp.com/bookNow", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            ...bookInfo,
            cardInfo: paymentMethod,
            serviceInfo: { description, price },
          }),
        })
          .then((res) => res.json())
          .then((data) => {
