...
const Testimonials = () => {
  ...
  useEffect(() => {
    fetch("https://redlight-server.herokuapp.com/reviews")
      .then((res) => res.json())
      ...
      <div className="container">
        <div className="testimonial-title">
          <h2 className="font-weight-bold text-center text-dark">
            <span className="text-danger">Testimonials</span>
          </h2>
        </div>
        <div className="testimonial-card-container">
          <div className="row py-5">
            {testimonial.map((td) => (
              <TestimonialCard key={td._id} testimonial={td}></TestimonialCard>
            ))}
          </div>
        </div>
      </div>
